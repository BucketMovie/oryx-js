...
describe('recommendations', function () {
    it('should get 10 recommendations for user 1', function (done) {
        OryxService.recommendation.get(1, 10, 0, function (err, res, body) {
            assert(!err);
    ...
    it('should get 10 recommendations for users 1 & 2', function (done) {
        OryxService.recommendation.getToMany([1, 2], 10, 0, function (err, res, body) {
            assert(!err);
            var data = parseCSV(body);
            assert.equal(data.length, 10);
            done();
    ...
    it('should get 8 recommendations for anonymous user', function (done) {
        ...
        OryxService.recommendation.getForAnonymous(scores, 8, 0, function (err, res, body) {
            assert(!err);
            var data = parseCSV(body);
